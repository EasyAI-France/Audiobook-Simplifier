...
def linkaudio(nom_sortie_name):
    ...
    output_file = sortie + "Combine_" + nom_sortie_name + ".wav"
...
def segmentation_txt(contenu):
    ...
    """
    Segmente le texte sur '.', '!' et '?', en conservant '!' et '?'
    mais en supprimant tous les points '.'.
    """
    # Segmente le texte sur tous les marqueurs de fin de phrase
    segments = re.findall(r'[^.!?]+[.!?]', contenu)

    # Supprime les points uniquement
    segments = [seg.replace('.', '') for seg in segments]
    ...
    if verif_doc == True:
        ...
        if nom_sortie_name == "":
            ...
            if memorie:
                ...
                function_voice(traite_fichier, lecture1, nom_sortie_name, audio, lang_chose,t_pause)
                print('Fin du traitement audio!')
...
with gr.Blocks(theme=gr.themes.Citrus()) as EasyAI:
    ...
    with gr.Group():
        ...
        with gr.Row(): 
            ...
            with gr.Column(scale=1):    
                gt_pause = gr.Textbox(label="Temps de la pause (Secondes)", value="1")
    ...
    run_button1.click(lambda nom_sortie_name: (linkaudio(nom_sortie_name), f"{sortie}Combine_{nom_sortie_name}.wav"), texte_input, [output1, audio_player])
...
verif_gpu()
print(device)
print('Ouvre cette URL Dans ton navigateur: 127.0.0.1:7860')
